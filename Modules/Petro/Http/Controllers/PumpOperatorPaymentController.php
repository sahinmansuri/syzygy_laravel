...
//         DB::raw('SUM(pump_operator_payments.payment_amount) as payment_amount'), // Fix: Sum payment amount to avoid duplicates
...
//         'pump_operator_other_sales.created_at as date_and_time',
//         DB::raw('NULL as collection_form_no'),
//         DB::raw('"Other Sale" as payment_type'),
//         DB::raw('SUM(pump_operator_other_sales.sub_total) as payment_amount'), // Fix: Sum sub_total to avoid duplicates
//         DB::raw('NULL as note'),
//         'pump_operators.name as pump_operator_name',
//         DB::raw('NULL as edited_by'),
//         'business_locations.name as location_name',
...
//     ->groupBy(
//         'pump_operator_other_sales.id',
//         'pump_operator_other_sales.created_at',
//         'pump_operators.name',
//         'business_locations.name',
//         'pump_operator_assignments.shift_number'
//     ); // Fix: Group by relevant fields
...
// if (!empty(request()->start_date) && !empty(request()->end_date)) {
//     $query->whereBetween('pump_operator_payments.date_and_time', [request()->start_date, request()->end_date]);
//     $query2->whereBetween('pump_operator_other_sales.created_at', [request()->start_date, request()->end_date]);
// }
...
$query = PumpOperatorPayment::leftJoin('pump_operators', 'pump_operator_payments.pump_operator_id', 'pump_operators.id')
    ...
    ->select(
        ...
        'pump_operator_payments.collection_form_no',
        'pump_operator_payments.payment_type',
        'pump_operator_payments.payment_amount', // Keep original value without summing
        'pump_operator_payments.note',
        'pump_operators.name as pump_operator_name',
        'edited_user.username as edited_by',
        'business_locations.name as location_name',
    ...
    ->groupBy(
        ...
        'pump_operator_payments.collection_form_no',
        'pump_operator_payments.payment_type',
        'pump_operator_payments.payment_amount', // Include in GROUP BY to keep original value
        'pump_operator_payments.note',
        'pump_operators.name',
        'edited_user.username',
        'business_locations.name',
...
$query2 = PumpOperatorOtherSale::join('pump_operator_assignments', 'pump_operator_assignments.shift_id', 'pump_operator_other_sales.shift_id')
    ...
    ->select(
        ...
        DB::raw('NULL as collection_form_no'),
        DB::raw('"Other Sale" as payment_type'),
        'pump_operator_other_sales.sub_total as payment_amount', // Keep original sub_total
        DB::raw('NULL as note'),
        'pump_operators.name as pump_operator_name',
        DB::raw('NULL as edited_by'),
        'business_locations.name as location_name',
    ...
    ->groupBy(
        'pump_operator_other_sales.id',
        'pump_operator_other_sales.created_at',
        'pump_operator_other_sales.sub_total', // Include in GROUP BY to avoid duplicate rows
        'pump_operators.name',
        'business_locations.name',
        'pump_operator_assignments.shift_number'
    );
