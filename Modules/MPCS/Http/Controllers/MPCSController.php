   ...
   public function get21CForms(Request $request)
    {
        ...
        //         $total_receipts =$this->Form21CQueryOpeningStock($business_id, $start_date, $end_date, $location_id);
            ...
            return view('mpcs::forms.partials.21c_details_section')->with(compact(
                'receipts',
                'todays',
                'previous_days',
                'opening_stocks',
                'cash_sales_todays',
                'issuess',
                'total_receipts_todates',
                'credit_sales',
                'corporate_section_todates',
              'total_issuess',
               'issue_upto_last_days',
               'total_issue_ones',
               'price_discount_todays',
               'price_discount_previouss',
               'total_discount_twos',
               'total_discount_one_twos',
               'balances',
                'subtotal_for_todays',
                 'pump_one_last_meters',
                'issue_qty_todays',
                'sub_categories',
                'start_date',
                'end_date',
...
return Datatables::of($rows)
    ...
    public function get21CForm(Request $request)
    {
        ...
        if (request()->ajax()) {
         ...
         return view('mpcs::forms.F21_form')->with(compact(
                    'credit_sales',
        ...
        public function Form21CQueryOpeningStock($business_id, $start_date, $end_date, $location_id)
    {
        ...
        $opening_stock = Transaction::leftjoin('purchase_lines', 'transactions.id', 'purchase_lines.transaction_id')
                ...
                ->where('transactions.business_id', $business_id)
                ->where('transactions.type', 'opening_stock')
                ->select('purchase_lines.quantity', 'purchase_lines.purchase_price', 'products.sub_category_id')
                ->get();
            ...
            ->leftjoin('business_locations', 'transactions.location_id', 'business_locations.id')
               ->leftjoin('account_transactions', 'transactions.id', '=', 'account_transactions.transaction_id')
                ...
                $query->where('account_transactions.type', 'debit')
                      ->orWhere('account_transactions.type', 'credit');
            })
             ->whereDate('transactions.transaction_date', '=', Carbon::now())
            ->select(
                'transactions.transaction_date',
