<?php $__env->startSection('title', __('lang_v1.customer_payments')); ?>
        ...
        <div class="row align-items-center">
            <div class="col-sm-6">
                <div class="breadcrumbs-area clearfix">
                    <h4 class="page-title pull-left"><?php echo app('translator')->get('lang_v1.customer_payments'); ?></h4>
                    <ul class="breadcrumbs pull-left" style="margin-top: 15px">
                        <li><a href="#"><?php echo app('translator')->get('lang_v1.customer_payments'); ?></a></li>
                        <li><span></span><?php echo app('translator')->get( 'contact.manage_your_contact', ['contacts' => __('lang_v1.customer_payments') ]); ?></li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    <section class="content main-content-inner">
        <div class="settlement_tabs">
            <ul class="nav nav-tabs">
                <?php if(!empty($pacakge_details['customer_payment_simple'])): ?>
                    <li class=" <?php if(empty(session('status.tab'))): ?> active <?php endif; ?>">
                        <a href="#customer_payment_simple" data-toggle="tab">
                            <i class="fa fa-money"></i> <strong><?php echo app('translator')->get('lang_v1.customer_payment_simple'); ?></strong>
                        </a>
                    </li>
                <?php endif; ?>
                
                <?php if(!empty($pacakge_details['customer_payment_bulk'])): ?>
                    <li class=" <?php if(session('status.tab') == 'bulk'): ?> active <?php endif; ?> <?php if(empty($pacakge_details['customer_payment_simple'])): ?> active <?php endif; ?>">
                        <a href="#customer_payment_bulk" data-toggle="tab">
                            <i class="fa fa-money"></i> <strong>
                                <?php echo app('translator')->get('lang_v1.customer_payment_bulk'); ?> </strong>
                        </a>
                    </li>
                <?php endif; ?>
                
                <?php if(!empty($pacakge_details['list_customer_payments'])): ?>
                    <li class=" <?php if(session('status.tab') == 'customer-payments'): ?> active <?php endif; ?>">
                        <a href="#customer_payments" data-toggle="tab">
                            <i class="fa fa-list"></i> <strong>
                                <?php echo app('translator')->get('lang_v1.customer_payments_list'); ?> </strong>
                        </a>
                    </li>
                <?php endif; ?>
                
                <?php if(!empty($pacakge_details['customer_interest'])): ?>
                    <li class=" <?php if(session('status.tab') == 'customer-interest'): ?> active <?php endif; ?>">
                        <a href="#customer_interest" data-toggle="tab">
                            <i class="fa fa-list"></i> <strong>
                                <?php echo app('translator')->get('lang_v1.customer_interest_list'); ?> </strong>
                        </a>
                    </li>
                <?php endif; ?>
                
                <?php if(!empty($pacakge_details['interest_settings'])): ?>
                    <li class=" <?php if(session('status.tab') == 'interest-settings'): ?> active <?php endif; ?>">
                        <a href="#interest_settings" data-toggle="tab">
                            <i class="fa fa-list"></i> <strong>
                                <?php echo app('translator')->get('lang_v1.interest_settings'); ?> </strong>
                        </a>
                    </li>
                <?php endif; ?>
            </ul>
            <div class="tab-content">
                <?php if(!empty($pacakge_details['customer_payment_simple'])): ?>
                    <div class="tab-pane <?php if(empty(session('status.tab'))): ?> active <?php endif; ?>" id="customer_payment_simple">
                        <?php echo $__env->make('customer_payments.customer_payment_simple', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                    </div>
                <?php endif; ?>
                <?php if(!empty($pacakge_details['customer_payment_bulk'])): ?>
                    <div class="tab-pane <?php if(session('status.tab') == 'bulk'): ?> active <?php endif; ?> <?php if(empty($pacakge_details['customer_payment_simple'])): ?> active <?php endif; ?>" id="customer_payment_bulk">
                        <?php echo $__env->make('customer_payments.customer_payment_bulk', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                    </div>
                <?php endif; ?>
                <?php if(!empty($pacakge_details['list_customer_payments'])): ?>
                    <div class="tab-pane <?php if(session('status.tab') == 'customer-payments'): ?> active <?php endif; ?>"
                         id="customer_payments">
                        <?php echo $__env->make('customer_payments.customer_payments', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                    </div>
                <?php endif; ?>
                <?php if(!empty($pacakge_details['customer_interest'])): ?>
                    <div class="tab-pane <?php if(session('status.tab') == 'customer-interest'): ?> active <?php endif; ?>"
                         id="customer_interest">
                        <?php echo $__env->make('customer_payments.customer_interest', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                    </div>
                <?php endif; ?>
                <?php if(!empty($pacakge_details['interest_settings'])): ?>
                    <div class="tab-pane <?php if(session('status.tab') == 'interest-settings'): ?> active <?php endif; ?>"
                         id="interest_settings">
                        <?php echo $__env->make('customer_payments.interest_settings', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                    </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    <script>
        ...
        $(document).on('input','.amount_column_total,#payable_amount',function(){
            ...
            if(total_amount < total_paid){
                toastr.error('<?php echo e(__("lang_v1.balance_is_insufficient"), false); ?>','Error');
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/vimi31/public_html/resources/views/customer_payments/index.blade.php ENDPATH**/ ?>